  ...
  function signInWithGoogle() {
    firebase
      ...
      .then((result) => {
        ...
        console.log({'username':result.user.email.split("@")[0],'email':result.user.email});
        axios.post("http://localhost:3000/api/user",{'username':result.user.email.split("@")[0],'email':result.user.email})
        .then(() => {
        console.log("Done"),
